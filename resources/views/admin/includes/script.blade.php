<script type="text/javascript">
  ...
  // notification function. refresh div without reload page
    ...
    //          });
    // }
    function load_intimation_notification() {
      count=0;
        $.ajax({
          method: "GET",
          url: "{{url('dashboard/intimation-notification')}}",
          success: function(data) {
            if(data.status==1)
             $.each(data.intimation, function( key, val ) {
              if(val>0){
                  ...
                  $("#intimation_notification").append('<li><a href="'+url+'"><div><strong>'+val+'</strong><span class="pull-right text-muted"><em>'+key+' days</em></span></div><div>Leads</div></a></li><li class="divider"></li>');
                }
