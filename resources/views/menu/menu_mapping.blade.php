...
$(document).ready(function() {
  $('.wbtn').click(function(){
   ...
   if($(this).prop('checked')) { flag=1;}else {flag=0;}
   readwrite('w',flag,men_id,menu_group_id);
...
$('.rbtn').click(function(){
 flag=0;
   ...
   menu_group_id=$('#menu_group_id').val();
   if($(this).prop('checked')) { flag=1;}else {flag=0;}
   readwrite('r',flag,men_id,menu_group_id);
});

function readwrite(check,flag,men_id,menu_group_id){
    ...
    token="{{csrf_token()}}";
     ...
     $.post("{{url('menu-rights')}}",data).done(function(respo){ 
